 ...
 * Decrypted data will hold user name & password for every new line with format like
 * <user name>:<password>\n
 */
 ...
 * Internally uses lckpwdf to acuquire the lock. Will try to acquire
 * the lock max for 100ms with 1ms interval.tion_Name.
 *
 ...
 * Function which will do the real encryption or decryption of the data.
 *
 * @param[in] pamh - pam handle.
 ...
 * @param[in] iv - Initial vectord data, used along with key
